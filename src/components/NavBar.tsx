...
export const navLinks = [
  { name: "Home", hash: "#home", icon: GoHome },
  { name: "Skills", hash: "#skills", icon: GoStack },
  { name: "Projects", hash: "#projects", icon: GoProject },
  { name: "About me", hash: "#about-me", icon: GoPerson },
  { name: "Contact", hash: "#contact", icon: GoMail },
];
...
const NavBar: React.FC = () => {
  ...
  const CustomNavLink: React.FC<CustomNavLinkProps> = ({
    ...
    const leftArrow = isLinkActive && (
      <span className="text-[--blue] absolute -left-5 top-0 max-lg:hidden">
        &lt;
      </span>
    );

    const rightArrow = isLinkActive && (
      <span className="text-[--blue] absolute top-0 -right-10 max-lg:hidden">
        /&gt;
      </span>
    );
   ...
   `}
        >
          {navLinks.map((link, index) => (
            <CustomNavLink key={index} link={link.hash} linkEn={link.name}>
              {link.name === activeSection ? (
                <div>
                  <span className="text-[--blue] absolute -left-5 top-0">
                    &lt;
                  </span>
                  {link.name}
                </div>
